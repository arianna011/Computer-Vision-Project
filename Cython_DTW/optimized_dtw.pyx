...
def DTW_cost_to_accum_cost_and_steps(C_in, parameter):
    '''
    Inputs
        C: The cost Matrix
    '''
    ...
    try:
        C = np.array(C_in, dtype=DTYPE_FLOAT)
    except TypeError:
...
@cython.boundscheck(False) # turn off bounds-checking for entire function
def DTW_get_path(np.ndarray[DTYPE_FLOAT_t, ndim=2] accumCost, np.ndarray[np.uint32_t, ndim=2] stepsForCost, parameter):
    '''
